...
export default function BdLeadHelp() {
  ...
  return (
    <div className="bd-content bd-lead-help-page">
      <div className="bd-lead-help-header">
        <div className="bd-lead-help-header__titles">
          <h1 className="bd-lead-help-page__title">{headerTitle}</h1>
          <p className="bd-lead-help-page__subtitle">{headerSubtitle}</p>
        </div>
        <div className="bd-lead-help-header__badge">
      ...
      <div className="bd-lead-help-layout">
        <div className="bd-lead-help-card">
          <h2 className="bd-lead-help-card__heading">Leads with users</h2>
          {leadsLoading ? (
            ...
            <div className="bd-lead-help-list" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {leads.map((l) => (
                <button
                  key={l.id}
                  type="button"
                  className="bd-lead-help-row"
                ...
                >
                  <div className="bd-lead-help-row__title">{l.job_title || 'Untitled role'}</div>
                  <div className="bd-lead-help-row__company">{l.company_name || '—'}</div>
                </button>
              ))}
            </div>
          )}
          ...
          {!selectedLeadId ? (
            <div style={{ background: theme.cardBg, borderRadius: 18, border: `1px solid ${theme.border}`, padding: 18, boxShadow: '0 2px 8px rgba(15,23,42,0.06)', minHeight: 260 }}>
              <p style={{ color: theme.textMuted, fontSize: 14, margin: 0 }}>
                Select a lead from the list to see that user’s messages and reply to them.
              </p>
            </div>
          ) : (
